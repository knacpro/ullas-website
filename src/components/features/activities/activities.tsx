...
export default function ActivitiesComponent() {
  const activities = [
    ...
    },
    {
      icon: PartyPopper,
      title: "Festivals",
      description: "All festivals like Holi, Raksha Bandhan, Diwali, Eid and Chirstmas are celebrated with full vigor and vivacity."
    },
    {
      icon: Building,
      title: "Visits",
        ...
        <div className="relative pt-8 px-4 md:px-6 lg:px-8 max-w-7xl mx-auto">
          ...
          {/* <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-12">
            {activities.map((activity, index) => (
              <Card key={index} className={index === activities.length - 1 ? "md:col-span-2 lg:col-span-1" : ""}>
                <CardContent className="p-6 text-center">
                  <div className="mb-4 flex justify-center">
                    <activity.icon className="w-12 h-12 text-primary" />
                  </div>
                  <h2 className="text-2xl font-serif italic mb-2">{activity.title}</h2>
